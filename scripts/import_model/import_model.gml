...
//vertex_format_add_colour()
var SMF_format=vertex_format_end()

if file_exists(filename+".buff")
{
	var buff=buffer_load(filename+".buff")
	return vertex_create_buffer_from_buffer(buff,SMF_format)
}

if !file_exists(filename+".obj"){show_debug_message("Failed to load model " + string(filename)); exit;}
show_debug_message("Loading obj file " + string(filename) + " into SMF");

modelBuffer = vertex_create_buffer()
...
file = file_text_open_read(filename+".obj");
...
v = real(string_copy(vertString, 1, string_pos("/", vertString) - 1));
n = real(string_delete(vertString, 1, string_pos("/", vertString)));}
vertex_position_3d(modelBuffer, vx[| v], vz[| v], vy[| v]);
vertex_normal(modelBuffer, nx[| n], nz[| n], ny[| n]);
vertex_texcoord(modelBuffer, tx[| t], 1 - ty[| t]);
vertex_color(modelBuffer, c_white,1);
}
file_text_close(file); 
vertex_end(modelBuffer);

buffer_save(buffer_create_from_vertex_buffer(modelBuffer,buffer_fixed,1),filename+".buff")
